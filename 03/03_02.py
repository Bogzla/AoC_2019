...
for s in wire1:
    ...
    for i in range(tr):
        ...
        ln += 1
        if not (x,y) in gr:
            gr[(x,y)] = ln  #Use tuple of x,y as dictionary key and store path length. Do not overwrite if path crosses as only interested in smallest path length for each coordinate
       # print(str(x) + ' - ' + str(y)) #debug
#wire 2
x = 0
y = 0
ln = 0
least = 1000000 #aribatrary starting high score for least signal delay
for s in wire2:
    tr = int(s[1:len(s)]) #wire length
    d = s[0:1] #in which direction
    for i in range(tr):
        if d == 'U':
            ...
            x += 1
        ln += 1
        if (x,y) in gr: #if x,y tuple is an existing key, then wire2 crosses wire1 here, find signal delay (combined path length)
            sd = gr[(x,y)] + ln
            if sd < least: #calculate manhattan distance and check against previous least
                least = sd
print(least)
