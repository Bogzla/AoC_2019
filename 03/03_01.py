...
for s in wire1:
    ...
    for i in range(tr):
        if d == 'U':
            ...
            x += 1
        gr[(x,y)] = 1  #Use tuple of x,y as dictionary key
...
for s in wire2:
    ...
    for i in range(tr):
        if d == 'U':
            ...
            x += 1
        if (x,y) in gr: #if x,y tuple is an existing key, then wire2 crosses wire1 here
            md = (abs(x)+abs(y))
            if md < least: #calculate manhattan distance and check against previous least
                least = md
print(least)
